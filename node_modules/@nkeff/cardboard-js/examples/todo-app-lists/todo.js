import { init, tag, allTags, mountPoint, each, isEmpty, } from '../../dist/cardboard.js';
...
const root = init().append(tag('(head)').append(...makeLinks()), styles());
...
div.mount(button('remove').clicked(() => removeAll()), button('add').clicked(() => addAll()));
div.mount(itemInput, addItemBtn).addClass('header');
div
    .mount(p('There are no items: ', todoCount).addClass('list-empty').hideIf(todoCount), each(todos, (item) => {
